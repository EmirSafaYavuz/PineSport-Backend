...
namespace Business.Authentication
{
    public abstract class AuthenticationProviderBase : IAuthenticationProvider
    {
        ...
        public virtual async Task<IDataResult<PineToken>> Verify(VerifyOtpCommand command)
        {
            ...
            if (login == null)
            {
                return new ErrorDataResult<PineToken>(Messages.InvalidCode);
            }
            ...
            return new SuccessDataResult<PineToken>(accessToken, Messages.SuccessfulLogin);
        }

        public abstract Task<LoginUserResult> Login(LoginUserCommand command);

        public abstract Task<PineToken> CreateToken(VerifyOtpCommand command);
